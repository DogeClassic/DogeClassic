...
#if defined(NDEBUG)
# error "Dogeclassic cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x607c5eb828be516cad0b301df53c0a747f9eccc9ffeb7845039ab3af56871897");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dogeclassic: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dogeclassic Signed Message:\n";
...
{
    // Dogeclassic: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Dogeclassic: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Dogeclassic: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Dogeclassic: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dogeclassic: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dogeclassic: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa9d424ddaf9f457fed5213f920c6d838e86b78a56ecf476317be56eb394360b8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "24/07/2021 doge Classic Born";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0480fd3115a38d7cf173a6e2c213dd2ef326e80b430559c968ca88161e1c1b47c2cc5ddff4edae3a4acc44cfeac5595d01a66de6139ebece853057547a784c2f26") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1627135832;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085374684;

        if (fTestNet)
        {
            block.nTime    = 1627135764;
            block.nNonce   = 385612412;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
       ...
       ///////////////////////////////////     
        ...
        assert(block.hashMerkleRoot == uint256("0xec78861477163a386351f51e437e85c825f230f8fce0d6fe9f6783fae44050f9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb3, 0xda }; // Dogeclassic: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DogeclassicMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DogeclassicMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DogeclassicMiner(CWallet *pwallet)
{
    printf("DogeclassicMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dogeclassic-miner");
    ...
    try { loop {
        ...
        printf("Running DogeclassicMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DogeclassicMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DogeclassicMiner, pwallet));
}
